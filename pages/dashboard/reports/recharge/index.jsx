...
const Index = () => {
  ...
  async function fetchPendingTransactions() {
    setLoading(true);
    await BackendAxios.get(`/api/admin/recharges/pending`)
      .then((res) => {
        setLoading(false);
  ...
  async function generateReport(userId) {
    ...
    await BackendAxios.get(
      `/api/admin/print-report?type=recharges&from=${
      ...
      .then((res) => {
        ...
        fileDownload(res.data, `RechargesReport.xlsx`);
      })
      .catch((err) => {
      ...
      await BackendAxios.post(`/api/admin/user/info/${Formik.values.userQuery}`)
        .then(async (result) => {
          ...
          await BackendAxios.get(
            pageLink
              ? pageLink
              : `/api/admin/recharges/${Formik.values.status}?from=${
                  Formik.values.from + (Formik.values.from && "T" + "00:00")
                }&to=${
  ...
  const actionCellRenderer = (params) => {
    function updateData() {
      setLoading(true);
      BackendAxios.post(`api/admin/${params?.data?.provider || "paysprint"}/recharge-status`, {
        ...
        .then((res) => {
          ...
          Toast({
            status: "success",
            description: `Recharge for phone ${params.data.phone_number} updated!`,
          });
  ...
  return (
    <>
      <Layout pageTitle={"Recharge Reports"}>
        ...
        <Stack p={4} spacing={8} w={"full"} direction={["column", "row"]}>
          <FormControl w={["full", "xs"]}>
            <FormLabel>Ref ID or Phone No.</FormLabel>
            <Input name="query" onChange={Formik.handleChange} bg={"white"} />
          </FormControl>
          <FormControl w={["full", "xs"]}>
            <FormLabel>User ID or User Phone</FormLabel>
            <Input
              name="userQuery"
              onChange={Formik.handleChange}
              bg={"white"}
